...
function _renderSummaryDetails(out, _results, limit, lvl) {
  ...
  const bps = _results.map((n) =>
    calcDiff(n.GuessRange.toArray(), n.MaxGrowBPs)
  );
  const countFull = {};
  const countFullPerp = [];
  let countSum = 0;
  ...
  const perp = 5;
  let possible = asMap(
    countFullPerp.filter((n) => n[1] > perp),
    (n) => n[0]
  );
  const _showResults = _results.filter(
    (n) => possible[calcDiff(n.GuessRange.toArray(), n.MaxGrowBPs).join(',')]
  );
...
function GuessResultToString(results, limit, showDetails) {
  ...
  if (_results.length) {
    ...
    if (fixed) {
      out.push(
        '總掉檔: ' + lostBP.join(' ~ ') + ' , 定檔 : \t' + ranges.join(' , ')
      );
    } else {
      out.push(
        '總掉檔: ' +
...
function _renderContent(
  ...
  data,
  widths,
  { lstart, lseparate, border, cseparate }
) {
  out.push('` ' + lstart);
