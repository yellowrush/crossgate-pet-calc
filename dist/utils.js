...
exports.GuessResultToString = exports.minmax = exports.fullRates = exports._loopForSum = exports.loopForSum = exports.calcDiff = exports.sum = void 0;
...
exports._loopForSum = _loopForSum;
...
function _renderSummaryDetails(out, _results, limit, lvl) {
    ...
    Object.keys(countFullPerp).forEach((n) => {
        countFullPerp.push([
            n,
            Math.round((countFullPerp[n] / countSum) * 1000) / 10,
        ]);
    });
    const perp = 5;
...
exports.GuessResultToString = GuessResultToString;
function mytable(datas, titleWidth) {
    ...
    for (let i = 0; i < datas.length; ++i) {
        for (let j = 0; j < datas[i].length; ++j) {
            let width = (0, string_width_1.default)(datas[i][j]);
            if (width % 2 == 1) {
                width++;
            }
...
function _renderContent(out, data, widths, { lstart, lseparate, border, cseparate }) {
    out.push('` ' + lstart);
    for (let i = 0; i < widths.length; ++i) {
        let width = (0, string_width_1.default)('' + data[i]);
        out.push(' ');
        out.push(data[i]);
...
function _renderline(out, widths, { lstart, lseparate, border, cseparate }) {
    out.push('` ' + lstart);
    out.push(border);
    for (let i = 0; i < widths.length; ++i) {
        out.push(border);
