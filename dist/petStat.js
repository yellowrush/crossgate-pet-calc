...
exports.GuessResultToString = exports.minmax = exports.calcDiff = exports.sumArray = exports.GrowRange = exports.Stat = exports.BP = exports.RealGuessRaw = exports.RealGuess = void 0;
const mathjs_1 = require("mathjs");
const utils_1 = require("./utils");
Object.defineProperty(exports, "calcDiff", { enumerable: true, get: function () { return utils_1.calcDiff; } });
Object.defineProperty(exports, "minmax", { enumerable: true, get: function () { return utils_1.minmax; } });
Object.defineProperty(exports, "GuessResultToString", { enumerable: true, get: function () { return utils_1.GuessResultToString; } });
class Stat {
    ...
    toBP() {
        ...
        const x = (0, mathjs_1.lusolve)(martrix, b);
    ...
    same(stat, tolerance = 0) {
        if (Math.abs((0, utils_1.sum)(this.toArray()) - (0, utils_1.sum)(stat.toArray())) > tolerance) {
            return false;
        }
        if ((0, utils_1.calcDiff)(this.toArray(), stat.toArray()).filter((n) => Math.abs(n) > tolerance).length) {
            return false;
        }
        return true;
...
class BP {
    ...
    calcRealNum() {
        ...
        const s = new Stat(0, fixPos(this.calcHP()) + propBase, fixPos(this.calcMP()) + propBase, fixPos(this.calcATK()) + propBase, fixPos(this.calcDEF()) + propBase, fixPos(this.calcAGI()) + propBase);
        return s;
    }
    str() {
        return ('hp:' +
...
class GrowRange {
    ...
    calcBPAtLevel(lvl, lvlpoint) {
        ...
        const lvldiff = lvl - 1;
        const shift = 0;
        bps[0] = bps[0] + utils_1.fullRates[this.hpp - shift] * lvldiff;
        bps[1] = bps[1] + utils_1.fullRates[this.attackp - shift] * lvldiff;
        bps[2] = bps[2] + utils_1.fullRates[this.defendp - shift] * lvldiff;
        bps[3] = bps[3] + utils_1.fullRates[this.agip - shift] * lvldiff;
        bps[4] = bps[4] + utils_1.fullRates[this.mpp - shift] * lvldiff;
        if (lvlpoint == null) {
            lvlpoint = lvldiff;
        }
        ...
        return {
            ...
            sumBaseBP: (0, utils_1.sum)(bps) + 10 * this.bprate,
            sumFullBP: (0, utils_1.sum)(bps) + 10 * this.bprate + lvlpoint,
        };
    }
    mockLoopRange(grow, cb) {
        cb((0, utils_1.sum)(this.bps()), grow);
    }
    loopRange(cb) {
        const sumBP = (0, utils_1.sum)(this.bps());
    ...
    guessWithSpecficLvlPoint(stat, point, targetGrow) {
        ...
        if (targetGrow) {
            ...
            this.mockLoopRange(this.drop.apply(this, targetGrow.toArray()), (sumBP, growRange) => {
                return this._handleGuessingGrowRange(growRange, stat, point, oSum, oUpSum, calcUpBp, results, result, sumBP);
            });
            return result;
        }
    ...
    _handleGuessingGrowRange(growRange, stat, point, oSum, oUpSum, calcUpBp, results, result, sumBP) {
        ...
        if (res.sumFullBP >= oSum && res.sumFullBP <= oUpSum) {
            const softLimit = (0, utils_1.calcDiff)(res.baseBP.toArray(), calcUpBp.toArray()).map((n) => n + 1);
            (0, utils_1.loopForSum)(point, 5, softLimit, (a, b, c, d, e) => {
                (0, utils_1.loopForSum)(10, 5, [10, 10, 10, 10, 10], (a1, b1, c1, d1, e1) => {
...
function possibleLostRange(growRange, maxBP) {
    ...
    const sumSureLost = (0, utils_1.sum)(sureLost);
    const sureBaseOver = [];
    for (var i = 0; i < guessRange.length; ++i) {
        if (guessRange[i] > maxBP[i]) {
    ...
    let sumSureBase = (0, utils_1.sum)(sureBaseOver);
...
function RealGuessRaw(Pts, input) {
    ...
    const [name, lvl, hp, mp, attack, def, agi, notorderpoint, targetGrow] = token;
    return RealGuess(Pts, name, lvl !== undefined ? parseInt(lvl) : undefined, hp !== undefined ? parseInt(hp) : undefined, mp !== undefined ? parseInt(mp) : undefined, attack !== undefined ? parseInt(attack) : undefined, def !== undefined ? parseInt(def) : undefined, agi !== undefined ? parseInt(agi) : undefined, notorderpoint !== undefined ? parseInt(notorderpoint) : undefined, targetGrow !== undefined ? targetGrow : undefined);
}
exports.RealGuessRaw = RealGuessRaw;
...
exports.RealGuess = RealGuess;
const sumArray = utils_1.sum;
